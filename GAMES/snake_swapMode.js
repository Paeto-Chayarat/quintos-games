...
function snakeSwap() {
	let swapped = new LinkedList();

	for (let s = snake.tail; s != null; s = s.prev) {
		// create copy of the snake part object to avoid
		// corrupting the original's data
		let copy = {
			block: s.block,
	...
	snake = swapped;
	// visually change the block type of the snake part
	snake.head.block = "snake head";
	snake.tail.block = "snake tail";
}
...
function draw() {
	...
	if (shouldSwap) {
		snakeSwap(); // swap every part in the snake

		// invert direction of all snake parts
		for (let s = snake.head; s != snake.tail; s = s.next) {
			if (s.direction == "up") s.direction = "down";
			else if (s.direction == "down") s.direction = "up";
			else if (s.direction == "left") s.direction = "right";
			else if (s.direction == "right") s.direction = "left";
		}
		inputDirection = head.direction;
		shouldSwap = false;
	}
}
