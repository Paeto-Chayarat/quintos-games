...
function aiPick() {
	...
	let ai0Btn = pc.button("AI Level 0", 60, 12, () => {
		aiLevel = 0;
		pick();
	});
	let ai1Btn = pc.button("AI Level 1", 60, 14, () => {
		aiLevel = 1;
		pick();
	});
	let ai2Btn = pc.button("AI Level 2", 60, 16, () => {
		aiLevel = 2;
		pick();
	});
	let ai3Btn = pc.button("AI Level 3", 60, 18, () => {
		aiLevel = 3;
		pick();
	});
	let aiCMBtn = pc.button("Challenge Mode", 60, 20, () => {
		aiLevel = 0;
		challengeMode = true;
		pick();
	});
